...
public class BTreeFile implements DbFile {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int holdNum = page.getMaxTuples() / 2;

		// the insertTuple function uses insertion sort. So I think using reverseIterator
		// may take more than iterator.
		...
		while (iter.hasNext()) {
			Tuple tp = iter.next();
			page.deleteTuple(tp);
			rightPage.insertTuple(tp);
		}

		Field midField = rightPage.iterator().next().getField(keyField);

		// getParentWithEmptySlots, this function take cares of the situation that we may need to
		// update the root pointer
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midField);

		// set parent id for rightPage and Page
		rightPage.setParentId(parent.getId());
		page.setParentId(parent.getId());

		// copy midField to parent
		BTreeEntry newEntry = new BTreeEntry(midField, page.getId(), rightPage.getId());
		parent.insertEntry(newEntry);

		// update the sibling id
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeInternalPage rPage = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);

		int holdNum = page.getMaxEntries() / 2 ;
		...
		while (iter.hasNext()) {
			...
			rPage.insertEntry(entry);
		}

		// rPage is a new page, all its children were moved.
		updateParentPointers(tid, dirtypages, rPage);

		// set middle entry and push it to the parent
		...
		midEntry.setRightChild(rPage.getId());
		parent.insertEntry(midEntry);

		// don't need to updateParentPointers for all children of parent.
		page.setParentId(parent.getId());
		rPage.setParentId(parent.getId());

		dirtypages.put(page.getId(), page);
		dirtypages.put(rPage.getId(), rPage);
		dirtypages.put(parent.getId(), parent);
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int tpNum = page.getNumTuples();
		int sblTpNum = sibling.getNumTuples();
		int moveNum = (sblTpNum - tpNum + 1) / 2;
		...
		// Question: Why don't update dirty pages in this function?
	}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int sblEntryNum = leftSibling.getNumEntries();
		int moveNum = (sblEntryNum - entryNum + 1) / 2;
		...
		// entries move to page, their children are moved.
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int entryNum = page.getNumEntries();
		int sblEntryNum = rightSibling.getNumEntries();
		int moveNum = (sblEntryNum - entryNum + 1) / 2;
