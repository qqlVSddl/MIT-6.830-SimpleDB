...
//        // some code goes here
        if (joinOp != Predicate.Op.EQUALS) {
            // range join, assume a fixed fraction of cross-product is emited by range scan.
            // a fraction of 0.3 is assumed here
            return card1 * card2 * 3 / 10;
        } else if (!t1pkey && !t2pkey) {
            // no primary key
//            return card1 * card2;
//            return 0;
            // only the following heuristic works.
...
//        }
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int len = 1; len <= joins.size(); ++len) {
            Set<Set<LogicalJoinNode>> subsets = enumerateSubsets(joins, len);
            for (Set<LogicalJoinNode> subset : subsets) {
                CostCard bestCC = new CostCard();
                bestCC.cost = Double.MAX_VALUE;
                for (LogicalJoinNode node : subset) {
                    CostCard currentCC = computeCostAndCardOfSubplan(stats, filterSelectivities,
                            node, subset, bestCC.cost, pc);
                    if (currentCC != null && currentCC.cost < bestCC.cost) {
                        bestCC = currentCC;
                    }
                }
                pc.addPlan(subset, bestCC.cost, bestCC.card, bestCC.plan);
            }
        }
        if (explain) {
